...
describe('CompressionInterceptor', () => {
  ...
  describe('intercept', () => {
    ...
    let mockResponse: { set: jest.Mock };
    ...
    it('should generate consistent ETag for same data', (done) => {
      ...
      result1.subscribe(() => {
        const firstCall = mockResponse.set.mock.calls.find(
          (call: [string, string]) => call[0] === 'ETag',
        );
        firstETag = firstCall![1];
        ...
        result2.subscribe(() => {
          const secondCall = mockResponse.set.mock.calls.find(
            (call: [string, string]) => call[0] === 'ETag',
          );
          const secondETag = secondCall![1];
  ...
  describe('private methods', () => {
    // Access private methods through bracket notation with proper typing
    interface CompressionInterceptorPrivate {
      isStaticResponse(data: unknown): boolean | undefined;
      generateETag(data: unknown): string;
    }

    let interceptorWithPrivateMethods: CompressionInterceptorPrivate;

    beforeEach(() => {
      interceptorWithPrivateMethods =
        interceptor as unknown as CompressionInterceptorPrivate;
    });
    ...
    describe('isStaticResponse', () => {
      it('should identify static responses correctly', () => {
        ...
        const result =
          interceptorWithPrivateMethods.isStaticResponse(staticData);
      ...
      it('should identify non-static responses correctly', () => {
        ...
        const result =
          interceptorWithPrivateMethods.isStaticResponse(dynamicData);
      ...
      it('should handle data without metadata', () => {
        ...
        const result =
          interceptorWithPrivateMethods.isStaticResponse(dataWithoutMetadata);
    ...
    describe('generateETag', () => {
      it('should generate valid ETag format', () => {
        const testData = { id: 123, name: 'Test' };
        const etag = interceptorWithPrivateMethods.generateETag(testData);
      ...
      it('should generate consistent ETags for same input', () => {
        const testData = { id: 123, name: 'Test' };
        const etag1 = interceptorWithPrivateMethods.generateETag(testData);
        const etag2 = interceptorWithPrivateMethods.generateETag(testData);
      ...
      it('should generate different ETags for different inputs', () => {
        const testData1 = { id: 123, name: 'Test1' };
        const testData2 = { id: 456, name: 'Test2' };
        const etag1 = interceptorWithPrivateMethods.generateETag(testData1);
        const etag2 = interceptorWithPrivateMethods.generateETag(testData2);
